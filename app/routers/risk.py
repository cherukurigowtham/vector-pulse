...
from app.core.helpers import (
    _key_preview, _resolve_risk_config, _sliding_window_rate_limit, 
    _log_event
)
...
async def explain_decision(risk_id: str, key_data: dict = Depends(require_api_key_or_admin)):
    try:
        ...
        if m.get("velocity"): narrative.append(f"High-frequency transaction wave detected ({config['velocity_max_orders']}+ orders in {config['velocity_window_secs']}s).")
        if m.get("vpn"):      narrative.append("Anonymous networking footprint identified (VPN/Proxy detected).")
        if m.get("price"):    narrative.append("Transaction value represents a significant statistical outlier compared to user history.")
        if m.get("sybil"):    narrative.append("Sybil identity pattern detected: multiple unique identifiers linked to a single delivery location.")
        if m.get("trust", 50.0) < config["trust_floor"]:
            narrative.append(f"Customer reputation score ({m['trust']:.0f}%) is below the merchant's delivery safety floor.")
...
async def _handle_outcome_feedback(risk_id: str, status: str, merchant_email: str):
    ...
    try:
        ...
        await apply_outcome_feedback(merchant_email, flags, status, identities=identities)
    except Exception as e:
...
async def update_outcome(update: OutcomeUpdate, bg: BackgroundTasks, key_data: dict = Depends(require_api_key)):
    try:
        ...
        merchant_email = key_data.get("email")
        bg.add_task(_handle_outcome_feedback, update.risk_id, update.status, merchant_email)
